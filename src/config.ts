...
export function resolveConfig(opts: { seat?: string }): Config {
  ...
  const apiBase = (process.env.PLANE_API_BASE ?? env("PLANE_API_BASE") ?? env("HOMETUTOR_TICKETS_API_BASE") ?? (url ? `${url.replace(/\/$/, "")}/api/v1` : "https://tools-small.tail8a19c.ts.net/api/v1")).replace(/\/$/, "");
